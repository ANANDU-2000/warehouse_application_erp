...
**Phase 1 status: COMPLETE.** Phase 2: **COMPLETE** (live SQL verified — `docs/44`). Phase 3: **COMPLETE** (platform). Phase 4: **Dashboard `/staff/home` STATES** on `ops/dashboard-module` — next COMPARE after approve.

### Task board (Dashboard `/staff/home`)

| State | Step |
|---|---|
| ✅ Completed | SCAFFOLD · LAYOUT · FIELDS · BUTTONS · WIRE (scoped) · STATES |
| 🟡 Current | STATES done — awaiting approve before COMPARE |
| ⬜ Pending | COMPARE |
| ⏸ Deferred | WIRE-2 bodies (warehouse stats, pending cards, shift feed, activity rows, notifications); Splash WIRE; Users & Roles implement |
...
| 2 | Dashboard | 🟡 `/staff/home` STATES | [`staff_home_states_compare.md`](modules/staff_home_states_compare.md) — next COMPARE |
...
**4.2 / 4.5 note:** Dashboard `/staff/home` STATES (`ops/dashboard-module`). Next: COMPARE. Splash WIRE deferred. Owner `/home` COMPARE PASS.
...
---
*Last updated: 2026-07-18 — Dashboard `/staff/home` STATES (`ops/dashboard-module`); next = COMPARE after approve. Splash WIRE deferred.*
