...
**Unlocked.** Phase 3 platform PASS. Login + Splash COMPARE PASS. Staff `/staff/home` WIRE-2 **COMPLETE**. Users & Roles Backend Slice 1–3 (`GET` list + `POST` + `GET` profile) **PASS**. Next: ask PATCH/mutate **or** Subagent 4 satellite.
...
**4.2 / 4.5 note:** Staff WIRE-2a–2f COMPLETE (`ops/dashboard-module`). Users list+create+profile APIs PASS — compare MDs. Ask before PATCH or one Subagent 4 satellite ([`dashboard_subagent4_inventory.md`](modules/dashboard_subagent4_inventory.md)). Users UI blocked. Owner routes + Splash COMPARE PASS.
