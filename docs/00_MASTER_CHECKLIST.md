...
**Phase 1 status: COMPLETE.** Phase 2: **COMPLETE** (live SQL verified — `docs/44`). Phase 3: **COMPLETE** (platform). Phase 4: staff WIRE-2 COMPLETE; Users & Roles Backend Slice 1–5 (list + create + profile + PATCH + DELETE) PASS on `ops/dashboard-module` — ask before next.

### Task board (Dashboard staff WIRE-2)

| State | Step |
|---|---|
| ✅ Completed | Login/Splash/home/staff COMPARE · WIRE-2a–2f · Users **Slice 1–5** (list + create + profile + PATCH + DELETE) |
| 🟡 Current | Users DELETE **PASS** — ask before reset-password **or** Subagent 4 satellite |
...
| 3 | Users & Roles | 🟡 Backend Slice 1–5 through DELETE PASS | list/create/profile/patch/delete compare MDs under `docs/modules/users_roles_backend_*` |
...
**Unlocked.** Phase 3 platform PASS. Login + Splash COMPARE PASS. Staff `/staff/home` WIRE-2 **COMPLETE**. Users & Roles Backend Slice 1–5 (through DELETE) **PASS**. Next: ask reset-password **or** Subagent 4 satellite.
...
**4.2 / 4.5 note:** Staff WIRE-2a–2f COMPLETE (`ops/dashboard-module`). Users list+create+profile+PATCH PASS — compare MDs. Ask before DELETE/reset or one Subagent 4 satellite ([`dashboard_subagent4_inventory.md`](modules/dashboard_subagent4_inventory.md)). Users UI blocked. Owner routes + Splash COMPARE PASS.
