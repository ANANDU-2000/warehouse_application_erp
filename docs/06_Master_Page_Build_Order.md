...
| 2 | Dashboard/Home | 2+ | ✅ `dashboard.md` | 🟡 WIRE-2 DONE | Next: ask Users GET profile **or** Subagent 4 satellite ([`dashboard_subagent4_inventory.md`](modules/dashboard_subagent4_inventory.md)) |
| 3 | Users & Roles | 2 | ✅ `users-roles.md` | 🟡 `GET`+`POST …/users` PASS | Profile/UI blocked — create compare [`users_roles_backend_create_compare.md`](modules/users_roles_backend_create_compare.md) |
...
Login UI: **COMPARE PASS**. Splash: **7 COMPARE PASS**. Dashboard `/home`+`/staff/home`+`/home/activity`+`/home/breakdown-more` COMPARE PASS. Staff **WIRE-2a–2f COMPLETE**. Users **`GET`+`POST …/users` PASS**. Next: ask **GET-by-id** or one **Subagent 4 satellite**. Users UI blocked until enough users APIs.
