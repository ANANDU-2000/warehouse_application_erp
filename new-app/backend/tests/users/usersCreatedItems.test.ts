/**
 * Users & Roles — GET …/users/:userId/created-items
...
describe("GET /v1/businesses/:businessId/users/:userId/created-items", () => {
  let users: UsersRepository;
  let memberships: MembershipsRepository;
  let businesses: BusinessesRepository;
  let businessUsers: BusinessUsersRepository;
  let listCreatedItemsByUser: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    listCreatedItemsByUser = vi.fn().mockResolvedValue([
      {
        id: ITEM_ID,
        name: "Rice 25kg",
        item_code: "BC-001",
  ...
  it("200 returns CreatedItemOut shape (barcode from item_code)", async () => {
    const res = await request(app())
      .get(`/v1/businesses/${BIZ_ID}/users/${TARGET_ID}/created-items`)
    ...
    expect(listCreatedItemsByUser).toHaveBeenCalledWith(BIZ_ID, TARGET_ID, 50);
  });

  it("200 empty array when none", async () => {
    listCreatedItemsByUser.mockResolvedValue([]);
    const res = await request(app())
      .get(`/v1/businesses/${BIZ_ID}/users/${TARGET_ID}/created-items`)
      .set("Authorization", `Bearer ${bearer()}`);
    expect(res.status).toBe(200);
  ...
  it("passes custom limit", async () => {
    const res = await request(app())
      .get(`/v1/businesses/${BIZ_ID}/users/${TARGET_ID}/created-items?limit=10`)
      .set("Authorization", `Bearer ${bearer()}`);
    expect(res.status).toBe(200);
    expect(listCreatedItemsByUser).toHaveBeenCalledWith(BIZ_ID, TARGET_ID, 10);
  });

  it("422 for limit=0", async () => {
    const res = await request(app())
      .get(`/v1/businesses/${BIZ_ID}/users/${TARGET_ID}/created-items?limit=0`)
      .set("Authorization", `Bearer ${bearer()}`);
    expect(res.status).toBe(422);
  });

  it("422 for limit=201", async () => {
    const res = await request(app())
      .get(`/v1/businesses/${BIZ_ID}/users/${TARGET_ID}/created-items?limit=201`)
      .set("Authorization", `Bearer ${bearer()}`);
    expect(res.status).toBe(422);
  });
