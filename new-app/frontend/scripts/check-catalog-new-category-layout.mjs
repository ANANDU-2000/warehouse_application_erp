/**
 * Catalog new category /catalog/new-category LAYOUT smoke.
 * Run: node scripts/check-catalog-new-category-layout.mjs
 * Source: catalog_add_category_page.dart · HexaColors · Outline radius 12
 */
...
assert(css.includes("--add-cat-gutter: 16px"), "gutter 16");
assert(css.includes("--add-cat-toolbar: 56px"), "toolbar 56");
...
assert(pkg.includes("test:catalog-new-category-layout"), "package script");

const r = spawnSync(
  process.execPath,
  [join(root, "scripts", "check-catalog-new-category-scaffold.mjs")],
...
if (failures.length) {
  console.error("Catalog new-category LAYOUT checks FAILED:");
  for (const f of failures) console.error(" -", f);
  process.exit(1);
}
console.log("Catalog new-category LAYOUT checks PASS");
