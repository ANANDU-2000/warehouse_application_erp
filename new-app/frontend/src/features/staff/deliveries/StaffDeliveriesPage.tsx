/**
 * Staff deliveries `/staff/deliveries` — SCAFFOLD (Step 1).
 * Source: staff_pending_deliveries_page.dart (StaffPendingDeliveriesPage).
 * Forbidden: handlers, API, Hexa polish (LAYOUT), section grouping (WIRE).
 * Deferred: scan → /barcode/scan; row → /staff/receive/:id (backend blocked).
 */
import {
...
export function StaffDeliveriesPage() {
  /* SCAFFOLD: fixed title without count; count when WIRE has total */
    ...
    >
      ...
      <main className="staff-del-body" data-slot="body">
        {SECTIONS.map((sec) => (
          ...
          >
            <h2 className="staff-del-section__title" data-slot="sectionTitle">
              {sec.title} (0)
            </h2>
            <div
