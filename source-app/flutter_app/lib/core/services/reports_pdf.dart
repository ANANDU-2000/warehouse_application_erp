      ...
      pw.Expanded(
        child: pw.Column(
          ...
          children: [
            ...
            pw.Text(
              safePdfText('Generated on: ${_genDf.format(DateTime.now())}'),
...
Future<PdfActionResult> shareReportsSummaryPdf({
  ...
  final header = await _businessPdfHeader(
    business,
    headline: safePdfText('Purchase Report | $modeSafe'),
    subline: pdfPeriodRange(_df.format(from), _df.format(to)),
  );
    ...
    pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        if (categoryRows != null && categoryRows.isNotEmpty) ...[
          ...
          _tableSection(
            ...
            rows: categoryRows
                .take(30)
                .map((r) => [
                      safePdfCell(r['category_name']?.toString()),
        ...
        if (supplierRows != null && supplierRows.isNotEmpty) ...[
          ...
          pw.SizedBox(height: 4),
          _tableSection(
            headers: ['Supplier', 'Deals', 'Total Rs.'],
            widths: [
            ...
            rows: supplierRows
                .take(30)
                .map((r) => [
                      safePdfCell(r['supplier_name']?.toString()),
        ...
        pw.Text(
          safePdfText(
            'Period: ${pdfPeriodRange(_df.format(from), _df.format(to))} | Generated by Harisree Exp&Pur',
          ),
          style: const pw.TextStyle(fontSize: 7.5, color: _muted),
        ),
      ],
...
Future<PdfActionResult> shareItemPurchaseTradeHistoryPdf({
  ...
  final header = await _businessPdfHeader(
    business,
    headline: safePdfText('Item statement - $cleanItem'),
...
Future<Uint8List> buildTradeStatementSsotPdfBytes({
  ...
  final header = await _businessPdfHeader(
    ...
    subline:
        'Period: ${pdfPeriodRange(_df.format(from), _df.format(to))} | ${purchases.length} purchases',
  );
                ...
                children: [
                  pw.TableRow(
                    ...
                    children: [
                      ...
                      _totCell(
                        'Total KG',
                        tt.kg > 1e-9 ? '${tt.kg.round()} KG' : pdfEmpty,
                        bold: true,
                      ),
                      _totCell(
                        'Total Amount',
